...
- [x] 整列補間（`CardAnimator`：手札スロットへ frame 非依存 lerp・ドロー時は山札位置から飛び込み）— 3件 PASS
- [x] 本家準拠の使用演出：単体対象カードは中央へ持ち上がり矢印で対象指示、確定で「上→墓地（捨て札）」へベジェ飛翔。非対象カードはカーソル追従で上方向リリース
- [ ] カード回転描画・整列補間の微調整＋実機での手触り調整
